pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/chknorrjks"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-ChuckNorrisJokesGenerator-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop chknorrjks | true"
                sh "docker rm chknorrjks | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name chknorrjks -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service chknorrjks-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"chknorrjks\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
